...
public class AchievementsHandler : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        achievements.Add("game_legend", new Achievement {Title = "Легенда игры", Target = 13, Award = 150});
        LoadAchievements();
    }
    ...
    public void UpdateProgress(string id, int amount)
    {
        if (achievements.TryGetValue(id, out Achievement ach))
        {
            ...
            {
                ach.IsUnlocked = true;
                PlayerPrefs.SetInt("HowMoneyAdds", PlayerPrefs.GetInt("HowMoneyAdds") + ach.Award);
